...
async function claimApprovedReturnCompletion(
  tx: any,
  id: string,
  organizationId: string,
  processedBy?: string,
) {
  // Atomic claim: only succeeds if the row is still APPROVED. Two parallel
  // POST /process calls → one wins, the other gets count=0 and we throw 409.
  ...
  // Persist audit columns (processed_at/by). Raw SQL because Prisma client
  // types may lag behind the migration that added these columns.
  // Best-effort: failure here doesn't roll back the claim.
  if (processedBy) {
    try {
      await tx.$executeRaw`
        UPDATE returns
           SET processed_at = NOW(),
               processed_by = ${processedBy}::uuid
         WHERE id = ${id}
      `;
    } catch (auditErr) {
      console.warn('[returns] processed_at/by write failed:', auditErr);
    }
  }
}
...
router.post('/',
  ...
  asyncHandler(async (req: EnhancedAuthenticatedRequest, res) => {
    ...
    // SECURITY: Re-resolve every unitPrice from the DB. The client cannot
    // be trusted to send the real price — accepting their value lets a
    // malicious caller drain the cash register by POSTing a return with
    // qty=1, unitPrice=999999. We map each request item to its
    // canonical sale_items row and use THAT price for the total + the
    // persisted return_items.
    ...
    // Idempotency: client_ref is a UUID generated by the frontend on the
    // FIRST submit and reused for retries. Two POSTs with the same key
    // return the same return row instead of duplicating. Migration
    // 20260515_returns_audit_columns.sql adds the unique index per org.
    const clientRefHeader = (req.headers['x-idempotency-key'] || req.headers['idempotency-key']);
    const clientRef = typeof clientRefHeader === 'string' && clientRefHeader.trim().length > 0
      ? clientRefHeader.trim()
      : null;

    if (clientRef) {
      // Use raw SQL because the Prisma client may not yet have the column
      // typed (it's generated from the schema and we just added it via
      // migration). The unique index handles the dedup atomically.
      ...
      if (existing.length > 0) {
        ...
        return res.status(200).json({ return: dup, deduplicated: true });
      }
    }
    ...
    const newReturn = await prisma.$transaction(async (tx) => {
      ...
      // Persist audit columns (cashier_id, client_ref, ip, ua) via raw SQL
      // because Prisma types lag behind the migration. Best-effort — failure
      // here doesn't roll back the return creation.
      try {
        const ip = (req.headers['x-forwarded-for'] as string)?.split(',')[0]?.trim() || req.ip || null;
        const ua = (req.headers['user-agent'] as string) || null;
        await tx.$executeRaw`
          UPDATE returns
             SET cashier_id = ${userId}::uuid,
        ...
        // Don't fail the return on audit-column write failures.
        console.warn('[returns] audit columns write failed:', auditErr);
      }
  ...
  asyncHandler(async (req: EnhancedAuthenticatedRequest, res) => {
    ...
    const updatedReturn = await prisma.$transaction(async (tx) => {
      if (status === 'COMPLETED' && returnRecord.status !== 'COMPLETED') {
        ...
        await claimApprovedReturnCompletion(tx, id, organizationId, req.user!.id);
      ...
      // Persist transition-specific audit columns. Raw SQL because the
      // Prisma client may not yet have these columns typed (added via
      // migration 20260515_returns_audit_columns.sql). Best-effort.
      try {
        if (status === 'APPROVED') {
          await tx.$executeRaw`
            UPDATE returns
               SET approved_at = NOW(),
                   approved_by = ${req.user!.id}::uuid
             WHERE id = ${id}
          `;
          ...
          await tx.$executeRaw`
            UPDATE returns
               SET rejected_at = NOW(),
                   rejected_by = ${req.user!.id}::uuid,
                   rejection_reason = ${notes || null}
             WHERE id = ${id}
          `;
        }
      } catch (auditErr) {
        console.warn('[returns] transition audit columns write failed:', auditErr);
      }
