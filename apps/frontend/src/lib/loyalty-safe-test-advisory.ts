...
// Database-level advisory lock implementation
async function acquireAdvisoryLock(lockKey: string, timeoutMs: number = 10000): Promise<AdvisoryLock | null> {
  try {
    const lockId = Math.abs(hashCode(lockKey))
    const { data, error } = await supabase.rpc('pg_advisory_lock_wrapper', { key: lockId })
    ...
    return new AdvisoryLock(supabase, lockId)
  } catch (error) {
...
class AdvisoryLock {
  ...
  async release(): Promise<void> {
    try {
      await this.supabase.rpc('pg_advisory_unlock_wrapper', { key: this.lockId })
    } catch (error) {
...
// Test implementations with database-level locking
async function redeemRewardSafe(rewardId: string, customerId: string, saleId?: string, idempotencyKey?: string) {
  const lock = await acquireAdvisoryLock(`reward:${rewardId}`, 15000)
...
async function adjustPointsSafe(customerId: string, delta: number, description?: string, idempotencyKey?: string) {
  const lock = await acquireAdvisoryLock(`points:${customerId}`, 10000)
...
async function closeCashSessionSafe(sessionId: string, actualCash: number, idempotencyKey?: string) {
  const lock = await acquireAdvisoryLock(`cash:${sessionId}`, 12000)
