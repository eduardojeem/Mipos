...
export async function POST(request: NextRequest) {
  try {
    ...
    const ids = Array.from(new Set(body.ids.map((value) => String(value).trim()).filter(Boolean)));
    ...
    if (body.action === 'delete') {
      ...
      const productConflicts = Array.from(
        new Set((productRows || []).map((row: any) => String(row.category_id)).filter(Boolean))
      );
      ...
      const childConflicts = Array.from(
        new Set((childRows || []).map((row: any) => String(row.parent_id)).filter(Boolean))
      );
