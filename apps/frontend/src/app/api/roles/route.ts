...
export async function GET(request: NextRequest) {
  ...
  const roles = rolesData.map((r: any) => {
    ...
    return {
        ...
        isSystem: ['ADMIN', 'SUPER_ADMIN', 'OWNER', 'SELLER', 'WAREHOUSE'].includes((r.name || '').toUpperCase()),
        isSystemRole: ['ADMIN', 'SUPER_ADMIN', 'OWNER', 'SELLER', 'WAREHOUSE'].includes((r.name || '').toUpperCase()),
        priority: 0,
