...
export default function ReturnsPage() {
  // 🔍 DEBUG: Log inmediato al cargar el módulo
...
function ReturnsPageContent() {
  ...
  // ✅ Sync activeTab with filters using useEffect to avoid infinite loop
  useEffect(() => {
    if (activeTab !== 'all') {
  ...
  }, [activeTab]); // Solo cuando activeTab cambia
  ...
  // 🔍 DEBUG: Log para verificar que el componente se monta y hace peticiones
  useEffect(() => {
    console.log('🔍 [ReturnsPage] Component mounted');
    console.log('🔍 [ReturnsPage] Filters:', filters.filters);
    console.log('🔍 [ReturnsPage] Page:', page, 'PageSize:', pageSize);
    console.log('🔍 [ReturnsPage] Returns:', returns?.length || 0);
    console.log('🔍 [ReturnsPage] IsLoading:', isLoading);
  }, [filters.filters, page, pageSize, returns, isLoading]);
