...
import { getPromotionStatus, impactLevel } from './utils';
...
export default function PromotionsPage() {
  ...
  const [promotionForProductModal, setPromotionForProductModal] = useState<Promotion | null>(null);
  const [selectedProductIds, setSelectedProductIds] = useState<string[]>([]);
  const [productSearch, setProductSearch] = useState('');
  const [selectedCategoryFilter, setSelectedCategoryFilter] = useState<string>('');
  const [showActiveOnly, setShowActiveOnly] = useState<boolean>(false);
  const [productSortBy, setProductSortBy] = useState<'name' | 'price' | 'category'>('name');
  ...
  const { data: allLinks = [], isPending: linksPending } = usePromotionLinks();
  const linkMutation = useLinkPromotionToTier();
  const unlinkMutation = useUnlinkPromotionFromTier();
  const [newTierIds, setNewTierIds] = useState<Record<string, string>>({});
  const [isApprovalDialogOpen, setIsApprovalDialogOpen] = useState(false);
  const [approvalTarget, setApprovalTarget] = useState<Promotion | null>(null);
  ...
  // Centralized store (Zustand) for promotions with TTL caching
  ...
  const promotionsSource = useStore(s => s.promotionsSource);
  ...
  useEffect(() => {
    ...
    const t = window.setInterval(() => { fetchStorePromotions().catch(() => { }); }, refreshIntervalMs);
    return () => { window.clearInterval(t); };
  }, [autoRefreshEnabled, refreshIntervalMs, fetchStorePromotions]);

  const [debouncedProductSearch, setDebouncedProductSearch] = useState('');
  useEffect(() => {
    const h = setTimeout(() => setDebouncedProductSearch(productSearch), 300);
    return () => clearTimeout(h);
  }, [productSearch]);

  useEffect(() => {
    try {
      const raw = localStorage.getItem('promotions:product-filters') || '';
      if (raw) {
        const saved = JSON.parse(raw);
        setSelectedCategoryFilter(saved?.category || '');
        setShowActiveOnly(!!saved?.activeOnly);
        setProductSearch(saved?.search || '');
      }
    } catch { }
  }, []);
  ...
  useEffect(() => {
    try {
      const payload = { category: selectedCategoryFilter || '', activeOnly: !!showActiveOnly, search: productSearch };
      localStorage.setItem('promotions:product-filters', JSON.stringify(payload));
    } catch { }
  }, [selectedCategoryFilter, showActiveOnly, productSearch]);
  ...
  const productsData: Product[] = useMemo(() => (productsHook.data || []) as any, [productsHook.data]);
  const processedProducts = useMemo(() => {
    ...
    return sorted;
  }, [productsData, productSearch, selectedCategoryFilter, showActiveOnly, productSortBy, productSortOrder]);

  // Keep local state in sync with store items (legacy parts still read local state)
  useEffect(() => {
    setPromotions(storeItems);
  }, [storeItems]);

  const fetchPromotions = async () => {
    await fetchStorePromotions();
  }

  // Display errors from store
  useEffect(() => {
    if (storeError) {
      toast({ title: 'Error', description: storeError, variant: 'destructive' });
    }
  }, [storeError, toast]);
  ...
  const fetchCarousel = async () => {
    try {
      const res = await api.get('/promotions/carousel');
      const ids: string[] = res.data?.ids || [];
      setCarouselIds(ids);
    } catch (e) {
      console.warn('No se pudo cargar el carrusel (se usará vacío):', (e as any)?.message);
    }
  }
  ...
  const moveFeaturedUp = (id: string) => {
    setCarouselIds((prev) => {
  ...
  const moveFeaturedDown = (id: string) => {
    setCarouselIds((prev) => {
      const idx = prev.indexOf(id);
  ...
  const handleSaveCarousel = async () => {
    try {
      ...
      const res = await api.put('/promotions/carousel', { ids: carouselIds });
      if (res.data?.success) {
        setCarouselIds(res.data?.ids || carouselIds);
        toast({ title: 'Carrusel actualizado', description: 'Se guardó el orden y selección de promociones.' });
      } else {
        throw new Error(res.data?.message || 'Respuesta inválida');
      }
    } catch (error: any) {
      toast({ title: 'Error', description: error?.message || 'No se pudo guardar el carrusel', variant: 'destructive' });
    } finally {
  ...
  const handleUpdatePromotion = async () => {
    ...
    try {
      const response = await api.put(`/promotions/${selectedPromotion.id}`, formData);
      if (response.data.success) {
        toast({
  ...
  const confirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await api.delete(`/promotions/${promotionToDelete.id}`);
      if (response.data.success) {
        toast({
  ...
  const handleTogglePromotionStatus = async (promotionId: string, isActive: boolean) => {
    try {
      const response = await api.patch(`/promotions/${promotionId}/status`, { isActive: !isActive });
      if (response.data.success) {
        toast({
  ...
  const handleSubmitApproval = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        toast({
          title: approvalStatus === 'approved' ? 'PromociÃ³n aprobada' : 'PromociÃ³n rechazada',
          description: 'El estado de aprobaciÃ³n se actualizÃ³ correctamente',
        })
      ...
      toast({
        title: 'Error',
        description: error?.response?.data?.message || 'No se pudo actualizar la aprobaciÃ³n',
        variant: 'destructive',
      })
    }
  }
  ...
  const openProductAdminForPromotion = (promotion: Promotion) => {
    // Preparar selecciÃ³n inicial con productos aplicables
  ...
  const handleSaveProductSelection = async () => {
    ...
    // Reutilizar la lÃ³gica de actualizaciÃ³n existente
    ...
    try {
      ...
      if (response.data.success) {
        toast({
          ...
          description: 'Se aplicÃ³ la selecciÃ³n de productos a la promociÃ³n',
        })
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.message || 'No se pudo actualizar los productos de la promociÃ³n',
        variant: 'destructive',
      })
    }
  }
  ...
  const validateCouponForm = () => {
    ...
    if (!/^[A-Z0-9]{8,12}$/.test(code)) return 'CÃ³digo invÃ¡lido';
    if (couponForm.type === 'PERCENTAGE') {
      if (!(couponForm.value > 0 && couponForm.value <= 100)) return 'Valor de porcentaje invÃ¡lido';
    } else {
      if (!(couponForm.value > 0)) return 'Valor fijo invÃ¡lido';
    }
    const sd = new Date(couponForm.startDate);
    const ed = new Date(couponForm.endDate);
    if (isNaN(sd.getTime()) || isNaN(ed.getTime())) return 'Fechas invÃ¡lidas';
    if (ed.getTime() <= sd.getTime()) return 'Fin debe ser posterior a inicio';
    if (coupons.some(c => c.code.toUpperCase() === code)) return 'CÃ³digo ya existe';
    return '';
  };
  ...
  const submitCoupon = async () => {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        fetchCoupons();
        toast({ title: 'CÃ³digo creado', description: 'El cÃ³digo se guardÃ³ correctamente.' });
      }
    } catch (e: any) {
      const m = e?.response?.data?.message || e?.message || 'Error al crear cÃ³digo';
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold">GestiÃ³n de Promociones</h1>
        </div>
  ...
  return (
    ...
    >
      <div className="space-y-6">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
              ...
              <DialogContent className="max-w-2xl" aria-labelledby="promotion-create-title">
                <DialogTitle className="sr-only">Crear Nueva PromociÃ³n</DialogTitle>
                <DialogHeader>
                  <DialogTitle id="promotion-create-title">Crear Nueva PromociÃ³n</DialogTitle>
                  <DialogDescription>
                    Completa la informaciÃ³n para crear una nueva promociÃ³n
                  </DialogDescription>
                </DialogHeader>

                <div className="grid gap-4 py-4 max-h-96 overflow-y-auto">
                  <div className="grid gap-2">
                    <Label htmlFor="name">Nombre de la PromociÃ³n</Label>
                    <Input
                      id="name"
                      value={formData.name}
                  ...
                  <div className="grid gap-2">
                    ...
                    <Textarea
                      ...
                      placeholder="Describe los tÃ©rminos y condiciones de la promociÃ³n"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label htmlFor="minPurchaseAmount">Compra MÃ­nima (opcional)</Label>
                      <Input
                    ...
                    <div className="grid gap-2">
                      <Label htmlFor="usageLimit">LÃ­mite de Uso (opcional)</Label>
                      <Input
                        id="usageLimit"
                        type="number"
                  ...
                  {formData.discountType === 'PERCENTAGE' && (
                    <div className="grid gap-2">
                      <Label htmlFor="maxDiscountAmount">Descuento MÃ¡ximo (opcional)</Label>
                      <Input
        ...
        <Card>
          <CardHeader>
            <CardTitle>CÃ³digos de Descuento</CardTitle>
            <CardDescription>Gestiona cÃ³digos, filtros y exporta listado</CardDescription>
          </CardHeader>
          <CardContent>
            {(() => {
              const now = new Date();
              ...
              return (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
                  ...
                  <div className="md:col-span-2 lg:col-span-1">
                    <div className="text-sm text-muted-foreground">PrÃ³ximos (â‰¤14 dÃ­as) / Recientes (â‰¤14 dÃ­as)</div>
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex gap-2">
                <Input placeholder="Buscar cÃ³digo" value={couponsSearch} onChange={(e) => setCouponsSearch(e.target.value)} className="w-48" />
              ...
              <div className="flex gap-2">
                <Dialog open={isCouponDialogOpen} onOpenChange={setIsCouponDialogOpen}>
                  <DialogTrigger asChild>
                    <Button onClick={() => setIsCouponDialogOpen(true)}>Nuevo CÃ³digo</Button>
                  </DialogTrigger>
                  <DialogContent className="max-w-xl" aria-labelledby="coupon-create-title" aria-describedby="coupon-create-desc">
                    <DialogTitle className="sr-only">Crear cÃ³digo de descuento</DialogTitle>
                    <DialogHeader>
                      <DialogTitle id="coupon-create-title">Crear CÃ³digo</DialogTitle>
                      <DialogDescription id="coupon-create-desc">Define el cÃ³digo, tipo, valor y vigencia</DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-3">
                      <div className="grid gap-2">
                        <Label htmlFor="coupon-code">CÃ³digo</Label>
                        <div className="flex gap-2">
            ...
            <div className="border rounded-md">
              {couponsLoading ? (
                ...
                <div className="p-6 text-sm text-muted-foreground">No hay cÃ³digos</div>
              ) : (
                  ...
                  {coupons.map((c) => (
                    <div key={c.code} className="p-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm">{formatDate(c.startDate)} â†’ {formatDate(c.endDate)}</span>
                        {(() => {
                          const st = getCouponStatus(c);
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              <AlertDialogTitle>Eliminar cÃ³digo</AlertDialogTitle>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Visualiza rÃ¡pidamente las ofertas activas y los productos afectados. Usa las acciones para ver todas o crear nuevas.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {/* AdministraciÃ³n de productos por promociÃ³n */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
              <div className="md:col-span-2 grid gap-2">
                <Label htmlFor="promotion-selector">Selecciona una promociÃ³n</Label>
                <Select onValueChange={(promotionId) => {
                  ...
                  <SelectTrigger id="promotion-selector">
                    <SelectValue placeholder="Elige una promociÃ³n" />
                  </SelectTrigger>
                  <SelectContent>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-gray-600 dark:text-gray-400 flex items-center gap-2">
                            ...
                            <span>{formatDate(promotion.startDate)} â€” {formatDate(promotion.endDate)}</span>
                          </div>
                          <div className="text-xs text-gray-600 dark:text-gray-400 flex items-center gap-2">
            ...
            {promotions.length === 0 && (
              <div className="text-sm text-muted-foreground">AÃºn no hay promociones. Crea una para comenzar.</div>
            )}
          </CardContent>
        </Card>
        ...
        <Dialog open={isProductModalOpen} onOpenChange={setIsProductModalOpen}>
          <DialogContent className="max-w-3xl" aria-labelledby="product-admin-title" aria-describedby="product-admin-desc">
            <DialogTitle className="sr-only">Administrar productos de la promociÃ³n</DialogTitle>
            <DialogHeader>
              <DialogTitle id="product-admin-title">Administrar productos de la promociÃ³n</DialogTitle>
              <DialogDescription id="product-admin-desc">
                Selecciona los productos a los que aplica esta promociÃ³n.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="grid gap-2 md:col-span-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-muted-foreground">Si el estado no estÃ¡ disponible, se muestran todos.</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-2">
                ...
                <Input
                  id="product-search"
                  placeholder="Nombre, categorÃ­aâ€¦"
                  value={productSearch}
            ...
            <DialogFooter>
              ...
              <Button onClick={handleSaveProductSelection} disabled={!promotionForProductModal}>Guardar selecciÃ³n</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            ...
            title={autoRefreshEnabled ? 'Desactivar auto-actualizaciÃ³n' : 'Activar auto-actualizaciÃ³n'}
          >
          ...
          <select
            ...
            aria-label="Intervalo de auto-actualizaciÃ³n"
          >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mt-4 space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              {linksPending && (
                                <span className="text-xs text-muted-foreground">Cargandoâ€¦</span>
                              )}
                            </div>
                            <div className="flex flex-wrap gap-2">
                              {allLinks.filter(l => l.promotionId === promotion.id).length === 0 && (
                                <Badge variant="outline" className="text-muted-foreground">Sin vÃ­nculos</Badge>
                              )}
                              {allLinks
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="max-w-2xl" aria-labelledby="promotion-edit-title">
            <DialogTitle className="sr-only">Editar PromociÃ³n</DialogTitle>
            <DialogHeader>
              <DialogTitle id="promotion-edit-title">Editar PromociÃ³n</DialogTitle>
              <DialogDescription>
                Modifica la informaciÃ³n de la promociÃ³n
              </DialogDescription>
            </DialogHeader>

            <div className="grid gap-4 py-4 max-h-96 overflow-y-auto">
              <div className="grid gap-2">
                <Label htmlFor="edit-name">Nombre de la PromociÃ³n</Label>
                <Input
                  id="edit-name"
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
        ...
        <Dialog open={isApprovalDialogOpen} onOpenChange={setIsApprovalDialogOpen}>
          <DialogContent className="max-w-md" aria-labelledby="promotion-approval-title">
            <DialogTitle className="sr-only">GestiÃ³n de aprobaciÃ³n</DialogTitle>
            <DialogHeader>
              <DialogTitle id="promotion-approval-title">
                {approvalStatus === 'approved' ? 'Aprobar promociÃ³n' : 'Rechazar promociÃ³n'}
              </DialogTitle>
              <DialogDescription>
                {approvalStatus === 'approved' ? 'AÃ±ade un comentario opcional para la aprobaciÃ³n.' : 'Indica el motivo del rechazo (opcional).'}
              </DialogDescription>
            </DialogHeader>
