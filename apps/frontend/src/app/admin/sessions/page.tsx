  ...
  return (
    <Card className="border-slate-200/80 bg-white/90 dark:border-slate-700/70 dark:bg-slate-900/85">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm text-slate-600 dark:text-slate-200">
          {title}
        </CardTitle>
      </CardHeader>
      <CardContent>
...
export default function SessionsPage() {
  ...
  const [userTerminationTarget, setUserTerminationTarget] =
  ...
  const exportSessions = async (format: "csv" | "json") => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/sessions/export?${params.toString()}`,
        {
          cache: "no-store",
          credentials: "include",
        },
      ...
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}));
        throw new Error(
          (payload as { error?: string }).error || "No se pudo exportar",
        );
      }
        ...
        <CardContent className="flex gap-3 p-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-amber-800 dark:text-amber-200">
              Esta seccion requiere permisos administrativos y un plan
              compatible.
            </p>
          </div>
        </CardContent>
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-300">
              Supervisa accesos, detecta riesgo operativo y corta sesiones
              dentro del alcance permitido por empresa.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              disabled={sessionsQuery.isLoading || isRefreshing}
            >
              {sessionsQuery.isLoading || isRefreshing ? (
            ...
            <Button
              variant="outline"
              onClick={() => void exportSessions("csv")}
              disabled={exporting !== null}
            >
              {exporting === "csv" ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
            ...
            <Button
              ...
              onClick={() =>
                void runSessionAction(
                  ...
                  "Las sesiones expiradas fueron limpiadas.",
                )
              }
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard
          ...
          helper={`${summary.uniqueUsers} usuarios con actividad visible`}
        />
        <StatCard
          title="Sesiones expiradas"
          value={summary.expired}
          helper="Dentro del filtro aplicado"
        />
        <StatCard
          title="Riesgo alto"
          value={summary.highRisk}
          helper="Requieren revision o cierre inmediato"
        />
      </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <CardDescription className="text-slate-600 dark:text-slate-300">
                Refina la vista por estado, riesgo y empresa.
              </CardDescription>
            </div>
            {hasFiltersApplied && (
              <Button
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => {
                setPage(1);
                setFilters((current) => ({ ...current, userRole: value }));
              }}
            >
              <SelectTrigger>
                <SelectValue />
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => {
                setPage(1);
                setFilters((current) => ({ ...current, deviceType: value }));
              }}
            >
              <SelectTrigger>
                <SelectValue />
        ...
        <Alert className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/20">
          ...
          <AlertDescription>
            {organizationsQuery.error instanceof Error
              ...
              : "La lista de empresas no esta disponible ahora mismo."}
          </AlertDescription>
        </Alert>
      )}
      ...
      <Card className="border-slate-200/80 bg-white/90 shadow-sm dark:border-slate-700/70 dark:bg-slate-900/85">
        <CardHeader>
          ...
          <CardDescription className="text-slate-600 dark:text-slate-300">
            Vista operativa de accesos recientes y controles inmediatos.
          </CardDescription>
        </CardHeader>
              ...
              <TableBody>
                {sessionsQuery.isLoading ? (
                  ...
                  <TableRow>
                    <TableCell
                      colSpan={6}
                      className="py-10 text-center text-sm text-slate-500 dark:text-slate-300"
                    >
                  ...
                  sessions.map((session) => {
                    ...
                    return (
                      <TableRow
                        key={session.id}
                        className="dark:hover:bg-slate-800/30"
                      >
                        <TableCell>
                          <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                              {session.userRole} - {session.ipAddress}
                            </p>
                          </div>
                        </TableCell>
                        <TableCell>
                          {session.isCurrent ? (
                            <Badge className="bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-200">
                              Actual
                            </Badge>
                          ) : session.isActive ? (
                        ...
                        <TableCell>
                          {session.riskLevel === "high" ? (
                            <Badge className="bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-200">
                              Alto
                            </Badge>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button
                              variant="ghost"
                              size="icon"
                              aria-label={`Ver detalles de ${session.userName}`}
                            ...
                            {session.isActive && !session.isCurrent && (
                              <Button
                                ...
                                onClick={() =>
                                  void runSessionAction(
                                    terminateSessionKey,
                                    `/api/admin/sessions/${session.id}/terminate`,
                                    "La sesion fue terminada.",
                                  )
                                }
                                disabled={
                                  pendingAction === terminateSessionKey ||
                                  pendingAction === terminateUserKey
                                }
                              >
                                {pendingAction === terminateSessionKey ? (
                                  <Loader2 className="h-4 w-4 animate-spin" />
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                size="sm"
                disabled={page <= 1 || sessionsQuery.isLoading}
                onClick={() => setPage((current) => Math.max(1, current - 1))}
              >
                Anterior
              </Button>
              <span className="text-sm text-slate-600 dark:text-slate-200">
                Pagina {Math.min(page, pageCount)} de {pageCount}
              </span>
              <Button
                variant="outline"
                size="sm"
                disabled={page >= pageCount || sessionsQuery.isLoading}
                onClick={() =>
      ...
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Informacion operativa para revisar riesgo, actividad y
              dispositivo.
            </DialogDescription>
          </DialogHeader>
          {selectedSession && (
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {selectedSession.userEmail || "Sin email"}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500 dark:text-slate-400">Rol</p>
                    <p className="font-medium">{selectedSession.userRole}</p>
                  </div>
                  <div>
              ...
              <Card className="border-slate-200/80 dark:border-slate-700/70">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base">Sesion</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <div>
                  ...
                  <div>
                    <p className="text-slate-500 dark:text-slate-400">IP</p>
                    <p className="font-mono text-xs">
                      {selectedSession.ipAddress}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500 dark:text-slate-400">Creada</p>
                    <p className="font-medium">
                      {formatFullDate(selectedSession.createdAt)}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500 dark:text-slate-400">
                    ...
                    <p className="font-medium">
                      {formatFullDate(selectedSession.lastActivityAt)}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500 dark:text-slate-400">Expira</p>
                    <p className="font-medium">
                      {formatFullDate(selectedSession.expiresAt)}
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-slate-200/80 dark:border-slate-700/70 md:col-span-2">
                ...
                <CardContent className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="md:col-span-3">
                    ...
                    <p className="mt-1 rounded-lg bg-slate-50 p-3 font-mono text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                      {selectedSession.userAgent || "Sin user agent"}
                    </p>
                  </div>
                </CardContent>
              </Card>
      ...
      <AlertDialog
        open={Boolean(userTerminationTarget)}
        onOpenChange={(open) => !open && setUserTerminationTarget(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cerrar sesiones del usuario</AlertDialogTitle>
            <AlertDialogDescription>
              Se cerraran todas las sesiones activas de{" "}
              {userTerminationTarget?.userName || "este usuario"} dentro del
              alcance visible de esta empresa.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {pendingAction ===
              `user:${userTerminationTarget?.userId || ""}` ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : (
