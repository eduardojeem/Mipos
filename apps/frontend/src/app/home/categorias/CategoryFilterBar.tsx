      ...
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasActiveFilters && (
              <Link href="/home/categorias" aria-label="Limpiar filtros">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs text-slate-500 dark:text-slate-400">
        <span>
          <span className="font-semibold text-slate-800 dark:text-slate-200">
            {hasActiveFilters ? visibleCategories : totalCategories}
          </span>{' '}
          {hasActiveFilters ? `de ${totalCategories} ` : ''}rubros
        </span>
        {totalOrganizations > 0 && (
          <span>
            <span className="font-semibold text-slate-800 dark:text-slate-200">
              {totalOrganizations.toLocaleString('es')}
            </span>{' '}
            empresas
          </span>
        )}
        {totalProducts > 0 && (
          <span>
            <span className="font-semibold text-slate-800 dark:text-slate-200">
              {totalProducts.toLocaleString('es')}
            </span>{' '}
            productos
          </span>
        )}
