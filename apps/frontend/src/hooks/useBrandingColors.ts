...
export function resolveBrandingColors(config?: BusinessConfig | null) {
    const branding: Partial<BusinessConfig['branding']> = config?.branding ?? {};
...
export function useBrandingColors() {
    ...
    const colors = useMemo(() => {
        return resolveBrandingColors(config);
    }, [config]);
