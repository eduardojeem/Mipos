  ...
  const activities: ActivityItem[] = [
    {
      ...
      metadata: { amount: 125.5, customerName: "María González" },
      status: "success",
      priority: "medium",
    },
    {
...
export function RecentActivity({
  ...
  const [activities, setActivities] = useState<ActivityItem[]>(
    initialData || generateMockActivity(),
  );
      ...
      <CardContent>
        <ScrollArea className="h-[600px] pr-4">
          <div className="space-y-6">
            {Object.entries(activityGroups).map(
                    ...
                    {groupActivities.map((activity, index) => (
                      <div key={activity.id} className="group">
                        <div className="flex items-start space-x-4 p-4 rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-md transition-all duration-200">
                          <div
                            className={`p-2 rounded-lg border ${getActivityColor(activity.status)}`}
                          >
                              ...
                              <div className="flex flex-wrap gap-3 mb-3">
                                ...
                                {activity.metadata.quantity && (
                                  <div className="flex items-center space-x-1 text-xs text-gray-500">
                                    ...
                                    <span>
                                      {activity.metadata.quantity} unidades
                                    </span>
                                  </div>
                                )}
