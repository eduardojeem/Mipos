...
function countActiveFilters(f: Filters) {
  ...
  if (f.categoryId) n++;
  if (f.showDeleted) n++;
  else if (f.isActive === false || f.isActive === undefined) n++;
...
export const ProductsFilters = memo(function ProductsFilters({
  ...
  if (filters.showDeleted) {
    chips.push({ key: 'deleted', label: 'Papelera', onRemove: () => onFilterChange({ showDeleted: false, isActive: true }) });
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={statusValue} onValueChange={handleStatusChange}>
          <SelectTrigger className={cn(
            ...
            filters.isActive === undefined && !filters.showDeleted && 'border-sky-400/50 bg-sky-50 text-sky-700 ring-1 ring-sky-400/20 dark:border-sky-700/50 dark:bg-sky-950/30 dark:text-sky-400',
