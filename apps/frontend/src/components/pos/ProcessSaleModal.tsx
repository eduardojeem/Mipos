...
import { AlertTriangle, CheckCircle, Percent, DollarSign, Receipt, ShoppingCart, Package2, CreditCard, Smartphone, Banknote, Wallet, QrCode, Truck, Trash2, Tag, Users, Search } from 'lucide-react';
...
interface ProcessSaleModalProps {
  ...
  onConfirm: (discount: number, discountType: DiscountType, paymentDetails?: { transferReference?: string; cashReceived?: number; change?: number }) => Promise<void> | void;
...
export default function ProcessSaleModal({
  ...
  const baseTotals = useMemo(() => calculateCartWithIva(cart, products, 0, 'FIXED_AMOUNT', config), [cart, products, config]);
  ...
  const totals = useMemo(() => calculateCartWithIva(cart, products, Number(composedDiscountTotal) || 0, 'FIXED_AMOUNT', config), [cart, products, composedDiscountTotal, config]);
  ...
  const handleConfirmClick = () => {
    ...
    showConfirmation({
      ...
      onConfirm: async () => {
        ...
        const paymentDetails: { transferReference?: string; cashReceived?: number; change?: number } = {};
        if (selectedPaymentMethod === PaymentMethod.TRANSFER && transferReference) {
          paymentDetails.transferReference = transferReference;
        }
        if (selectedPaymentMethod === PaymentMethod.CASH && !splitPaymentEnabled) {
          paymentDetails.cashReceived = Number(cashReceived);
          paymentDetails.change = changeDue;
        }

        await onConfirm(Number(composedDiscountTotal) || 0, 'FIXED_AMOUNT', paymentDetails);
      },
    });
  };
  ...
  }, [isOpen, totals.total, onClose, handleConfirmClick]);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          {/* Customer Selection */}
          <Card className={cn("animate-in fade-in-50 transition-colors", customer ? "border-primary/30 dark:border-primary/40 bg-primary/5 dark:bg-primary/10" : "border-dashed dark:border-dashed")}>
            <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={cn("p-2.5 rounded-full flex-shrink-0", customer ? "bg-primary dark:bg-primary text-primary-foreground dark:text-primary-foreground" : "bg-muted dark:bg-muted text-muted-foreground dark:text-muted-foreground")}>
                  <Users className="h-5 w-5" />
                </div>
                <div>
                  <p className="text-xs font-medium text-muted-foreground dark:text-muted-foreground uppercase tracking-wider">Cliente de la Venta</p>
                  <div className="flex items-center gap-2">
                    <p className="font-bold text-lg leading-none text-foreground dark:text-foreground">{customer ? customer.name : 'Cliente General'}</p>
                    {customer?.customer_type === 'WHOLESALE' && (
                      <Badge variant="secondary" className="bg-orange-100 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400 hover:bg-orange-200 dark:hover:bg-orange-900/40 border-orange-200 dark:border-orange-800">Mayorista</Badge>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <Popover open={customerSearchOpen} onOpenChange={setCustomerSearchOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-full sm:w-[250px] justify-between bg-background dark:bg-background">
                      {customer ? (
                        <span className="truncate text-foreground dark:text-foreground">{customer.name}</span>
                      ) : (
                        <span className="text-muted-foreground dark:text-muted-foreground">Buscar o cambiar cliente...</span>
                      )}
                    ...
                    <Command>
                      ...
                      <CommandGroup className="max-h-[200px] overflow-y-auto">
                        ...
                        >
                          ...
                          {customer === null && <CheckCircle className="ml-auto h-4 w-4 opacity-50" />}
                        </CommandItem>
                        {customers.map((c) => (
                          <CommandItem
                            key={c.id}
                          ...
                          >
                            ...
                            <span>{c.name}</span>
                            {customer?.id === c.id && <CheckCircle className="ml-auto h-4 w-4 text-primary" />}
                          </CommandItem>
                        ))}
                      </CommandGroup>
                    </Command>
          ...
          <Card className="animate-in fade-in-50 bg-card dark:bg-card border-border dark:border-border">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2 text-foreground dark:text-foreground">
                ...
                Paso 1: Productos en el Carrito
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {cart.length === 0 ? (
                  <div className="flex items-center gap-2 p-3 rounded-md bg-muted/40 dark:bg-muted/20 text-sm text-muted-foreground dark:text-muted-foreground">
                  ...
                  cart.map((item, idx) => {
                    ...
                    return (
                      <div key={`${item.product_id}-${idx}`} className="flex items-center justify-between py-2 border-b dark:border-border last:border-0">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className="w-10 h-10 rounded-md overflow-hidden bg-muted dark:bg-muted flex items-center justify-center flex-shrink-0">
                            {productDetails?.image_url ? (
                              ...
                              <Package2 className="h-5 w-5 text-muted-foreground dark:text-muted-foreground" />
                            )}
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <div className="font-medium truncate text-foreground dark:text-foreground">{item.product_name}</div>
                              {hasPromo && (
                                <Badge variant="secondary" className="text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800">Promoción</Badge>
                              )}
                              {insufficient && (
                                <Badge variant="destructive" className="text-[10px] px-1.5 py-0.5">Stock insuficiente</Badge>
                              )}
                            </div>
                            <div className="text-xs text-muted-foreground dark:text-muted-foreground">Cantidad: {item.quantity}</div>
                            {insufficient && (
                              <div className="text-xs text-destructive dark:text-destructive">Disponible: {available}</div>
                            )}
                          </div>
                        </div>
                        <div className="w-24 text-right">
                          <div className="text-sm text-foreground dark:text-foreground">{fmtCurrency(item.price)}</div>
                          <div className="text-xs text-muted-foreground dark:text-muted-foreground">Unitario</div>
                        </div>
                        <div className="w-28 text-right">
                          <div className="text-sm font-semibold text-foreground dark:text-foreground">{fmtCurrency(item.total)}</div>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <button
                  ...
                  className={cn(
                    "relative flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 transition-all duration-300 overflow-hidden group",
                    "hover:scale-105 active:scale-95",
                    selectedPaymentMethod === PaymentMethod.CASH
                      ? "border-green-500 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-950/30 dark:to-green-900/30 shadow-lg shadow-green-500/20 ring-4 ring-green-500/20"
                      : "border-gray-200 dark:border-gray-700 hover:border-green-300 hover:bg-green-50/50 dark:hover:bg-green-950/10"
                  )}
                >
                  {/* Background gradient overlay */}
                  {selectedPaymentMethod === PaymentMethod.CASH && (
                    <div className="absolute inset-0 bg-gradient-to-br from-green-400/10 to-green-600/10 animate-pulse" />
                  )}
                  ...
                  {selectedPaymentMethod === PaymentMethod.CASH && (
                    <div className="absolute top-2 right-2 z-20">
                      <div className="bg-green-600 rounded-full p-1 shadow-lg animate-in zoom-in duration-200">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedPaymentMethod === PaymentMethod.CARD
                      ? "border-blue-500 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950/30 dark:to-blue-900/30 shadow-lg shadow-blue-500/20 ring-4 ring-blue-500/20"
                      : "border-gray-200 dark:border-gray-700 hover:border-blue-300 hover:bg-blue-50/50 dark:hover:bg-blue-950/10"
                  )}
                >
                  {selectedPaymentMethod === PaymentMethod.CARD && (
                  ...
                  {selectedPaymentMethod === PaymentMethod.CARD && (
                    <div className="absolute top-2 right-2 z-20">
                      <div className="bg-blue-600 rounded-full p-1 shadow-lg animate-in zoom-in duration-200">
                        <CheckCircle className="h-4 w-4 text-white" fill="currentColor" />
                      </div>
                    </div>
                  )}
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedPaymentMethod === PaymentMethod.TRANSFER
                      ? "border-purple-500 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-950/30 dark:to-purple-900/30 shadow-lg shadow-purple-500/20 ring-4 ring-purple-500/20"
                      : "border-gray-200 dark:border-gray-700 hover:border-purple-300 hover:bg-purple-50/50 dark:hover:bg-purple-950/10"
                  )}
                >
                  {selectedPaymentMethod === PaymentMethod.TRANSFER && (
                    <div className="absolute inset-0 bg-gradient-to-br from-purple-400/10 to-purple-600/10 animate-pulse" />
                  )}
                  ...
                  {selectedPaymentMethod === PaymentMethod.TRANSFER && (
                    <div className="absolute top-2 right-2 z-20">
                      <div className="bg-purple-600 rounded-full p-1 shadow-lg animate-in zoom-in duration-200">
                        <CheckCircle className="h-4 w-4 text-white" fill="currentColor" />
                      </div>
                    </div>
                  )}
                ...
                <button
                  ...
                  className={cn(
                    "relative flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 transition-all duration-300 overflow-hidden group",
                    "hover:scale-105 active:scale-95",
                    selectedPaymentMethod === PaymentMethod.OTHER
                      ? "border-gray-500 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800/30 dark:to-gray-700/30 shadow-lg shadow-gray-500/20 ring-4 ring-gray-500/20"
                      : "border-gray-200 dark:border-gray-700 hover:border-gray-400 hover:bg-gray-50/50 dark:hover:bg-gray-800/10"
                  )}
                >
                  {selectedPaymentMethod === PaymentMethod.OTHER && (
                    <div className="absolute inset-0 bg-gradient-to-br from-gray-400/10 to-gray-600/10 animate-pulse" />
                  )}

                  <Wallet className={cn(
                    "h-8 w-8 transition-all duration-300 relative z-10",
                    selectedPaymentMethod === PaymentMethod.OTHER
                      ? "text-gray-700 dark:text-gray-300 scale-110"
                      : "text-gray-600 dark:text-gray-400 group-hover:text-gray-700"
                  )} />
                  ...
                  {selectedPaymentMethod === PaymentMethod.OTHER && (
                    <div className="absolute top-2 right-2 z-20">
                      <div className="bg-gray-600 rounded-full p-1 shadow-lg animate-in zoom-in duration-200">
                        <CheckCircle className="h-4 w-4 text-white" fill="currentColor" />
                      </div>
                    </div>
                  )}
              ...
              {selectedPaymentMethod && (
                <div className="mt-4 p-4 rounded-lg bg-muted/50 border border-border animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="flex items-start gap-3">
                    <div className={cn(
                      "p-2 rounded-lg",
                      selectedPaymentMethod === PaymentMethod.CASH && "bg-green-100 dark:bg-green-950/30",
                      selectedPaymentMethod === PaymentMethod.CARD && "bg-blue-100 dark:bg-blue-950/30",
                      selectedPaymentMethod === PaymentMethod.TRANSFER && "bg-purple-100 dark:bg-purple-950/30",
                      selectedPaymentMethod === PaymentMethod.OTHER && "bg-gray-100 dark:bg-gray-800/30"
                    )}>
                      {selectedPaymentMethod === PaymentMethod.CASH && <Banknote className="h-5 w-5 text-green-600 dark:text-green-400" />}
                      {selectedPaymentMethod === PaymentMethod.CARD && <CreditCard className="h-5 w-5 text-blue-600 dark:text-blue-400" />}
                      {selectedPaymentMethod === PaymentMethod.TRANSFER && <QrCode className="h-5 w-5 text-purple-600 dark:text-purple-400" />}
                      {selectedPaymentMethod === PaymentMethod.OTHER && <Wallet className="h-5 w-5 text-gray-600 dark:text-gray-400" />}
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-foreground mb-1">
                        {selectedPaymentMethod === PaymentMethod.CASH && "Pago en Efectivo"}
                        {selectedPaymentMethod === PaymentMethod.CARD && "Pago con Tarjeta"}
                        {selectedPaymentMethod === PaymentMethod.TRANSFER && "Transferencia Bancaria"}
                        {selectedPaymentMethod === PaymentMethod.OTHER && "Otro Método de Pago"}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {selectedPaymentMethod === PaymentMethod.CASH && "El cliente pagará con dinero en efectivo. Asegúrate de tener cambio disponible."}
                        {selectedPaymentMethod === PaymentMethod.CARD && "El cliente pagará con tarjeta de débito o crédito. Verifica que el terminal esté funcionando."}
                        {selectedPaymentMethod === PaymentMethod.TRANSFER && "El cliente realizará una transferencia bancaria. Proporciona los datos de la cuenta."}
                        {selectedPaymentMethod === PaymentMethod.OTHER && "El cliente utilizará un método de pago alternativo."}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {/* Transfer Reference Section (solo para transferencia) */}
          {selectedPaymentMethod === PaymentMethod.TRANSFER && !splitPaymentEnabled && (
            <Card className="animate-in fade-in-50">
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="space-y-1">
                    <Label htmlFor="transfer-reference">Número de Comprobante / Referencia</Label>
                    <Input
                      id="transfer-reference"
                      type="text"
                      value={transferReference}
                      onChange={(e) => setTransferReference(e.target.value)}
                      placeholder="Ej: 123456789 o REF-2024-001"
                      className="h-10"
                    />
                    <div className="text-xs text-muted-foreground">
                      Ingresa el número de comprobante o referencia de la transferencia bancaria
                    </div>
                  </div>
                  {transferReference && (
