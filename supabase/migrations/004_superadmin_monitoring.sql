...
BEGIN
  RETURN QUERY
  SELECT 'products'::text, count(*) FROM products WHERE organization_id = org_id
  UNION ALL
  SELECT 'customers'::text, count(*) FROM customers WHERE organization_id = org_id
  UNION ALL
  SELECT 'suppliers'::text, count(*) FROM suppliers WHERE organization_id = org_id
  UNION ALL
  SELECT 'sales'::text, count(*) FROM sales WHERE organization_id = org_id
  UNION ALL
  SELECT 'sale_items'::text, count(*) FROM sale_items WHERE organization_id = org_id;
END;
$$ LANGUAGE plpgsql;
