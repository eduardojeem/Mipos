import { createClient } from '@supabase/supabase-js'
import dotenv from 'dotenv'

// Cargar variables de entorno
dotenv.config({ path: '.env.local' })

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!,
  {
    auth: {
      autoRefreshToken: false,
      persistSession: false
    }
  }
)

async function insertDataNatively() {
  console.log('🚀 INSERCIÓN NATIVA DE DATOS EN SUPABASE')
  console.log('=' .repeat(60))

  let successCount = 0
  let errorCount = 0
  const errors: string[] = []

  // Función auxiliar para insertar datos
  async function insertData(tableName: string, data: any[], description: string) {
    console.log(`📝 ${description}...`)
    try {
      const { data: result, error } = await supabase
        .from(tableName)
        .insert(data)
        .select()

      if (error) {
        console.log(`❌ Error: ${error.message}`)
        errors.push(`${description}: ${error.message}`)
        errorCount++
        return false
      } else {
        console.log(`✅ ${description} - ${result?.length || 0} registros insertados`)
        successCount++
        return true
      }
    } catch (err: any) {
      console.log(`❌ Excepción: ${err.message}`)
      errors.push(`${description}: ${err.message}`)
      errorCount++
      return false
    }
  }

  // 1. Insertar roles
  console.log('\n🔐 INSERTANDO ROLES')
  console.log('-' .repeat(40))
  
  const rolesData = [
    { name: 'SUPER_ADMIN', description: 'Administrador con acceso completo al sistema' },
    { name: 'ADMIN', description: 'Administrador con permisos limitados' },
    { name: 'MANAGER', description: 'Gerente con acceso a reportes y gestión' },
    { name: 'CASHIER', description: 'Cajero con acceso a ventas' },
    { name: 'INVENTORY_CLERK', description: 'Encargado de inventario' }
  ]

  await insertData('roles', rolesData, 'Insertando roles')

  // 2. Insertar permisos
  console.log('\n🔑 INSERTANDO PERMISOS')
  console.log('-' .repeat(40))
  
  const permissionsData = [
    { name: 'users.create', description: 'Crear usuarios' },
    { name: 'users.read', description: 'Leer usuarios' },
    { name: 'users.update', description: 'Actualizar usuarios' },
    { name: 'users.delete', description: 'Eliminar usuarios' },
    { name: 'products.create', description: 'Crear productos' },
    { name: 'products.read', description: 'Leer productos' },
    { name: 'products.update', description: 'Actualizar productos' },
    { name: 'products.delete', description: 'Eliminar productos' },
    { name: 'sales.create', description: 'Crear ventas' },
    { name: 'sales.read', description: 'Leer ventas' },
    { name: 'sales.update', description: 'Actualizar ventas' },
    { name: 'sales.delete', description: 'Eliminar ventas' },
    { name: 'purchases.create', description: 'Crear compras' },
    { name: 'purchases.read', description: 'Leer compras' },
    { name: 'purchases.update', description: 'Actualizar compras' },
    { name: 'purchases.delete', description: 'Eliminar compras' },
    { name: 'customers.create', description: 'Crear clientes' },
    { name: 'customers.read', description: 'Leer clientes' },
    { name: 'customers.update', description: 'Actualizar clientes' },
    { name: 'customers.delete', description: 'Eliminar clientes' },
    { name: 'suppliers.create', description: 'Crear proveedores' },
    { name: 'suppliers.read', description: 'Leer proveedores' },
    { name: 'suppliers.update', description: 'Actualizar proveedores' },
    { name: 'suppliers.delete', description: 'Eliminar proveedores' },
    { name: 'reports.view', description: 'Ver reportes' },
    { name: 'reports.export', description: 'Exportar reportes' },
    { name: 'system.settings', description: 'Configuración del sistema' },
    { name: 'system.backup', description: 'Respaldo del sistema' }
  ]

  await insertData('permissions', permissionsData, 'Insertando permisos')

  // 3. Insertar categorías
  console.log('\n📂 INSERTANDO CATEGORÍAS')
  console.log('-' .repeat(40))
  
  const categoriesData = [
    { name: 'Electrónicos', description: 'Dispositivos electrónicos y accesorios' },
    { name: 'Ropa', description: 'Prendas de vestir y accesorios' },
    { name: 'Hogar', description: 'Artículos para el hogar' },
    { name: 'Deportes', description: 'Equipos y accesorios deportivos' },
    { name: 'Libros', description: 'Libros y material educativo' }
  ]

  await insertData('categories', categoriesData, 'Insertando categorías')

  // 4. Insertar proveedores
  console.log('\n🏢 INSERTANDO PROVEEDORES')
  console.log('-' .repeat(40))
  
  const suppliersData = [
    { 
      name: 'TechSupply Corp', 
      contact_name: 'Juan Pérez', 
      email: 'juan@techsupply.com', 
      phone: '+1234567890',
      address: 'Calle Tech 123'
    },
    { 
      name: 'Fashion Wholesale', 
      contact_name: 'María García', 
      email: 'maria@fashion.com', 
      phone: '+1234567891',
      address: 'Av. Moda 456'
    },
    { 
      name: 'Home & Garden Ltd', 
      contact_name: 'Carlos López', 
      email: 'carlos@homeandgarden.com', 
      phone: '+1234567892',
      address: 'Plaza Hogar 789'
    }
  ]

  await insertData('suppliers', suppliersData, 'Insertando proveedores')

  // 5. Insertar clientes
  console.log('\n👥 INSERTANDO CLIENTES')
  console.log('-' .repeat(40))
  
  const customersData = [
    { 
      name: 'Ana Rodríguez', 
      email: 'ana@email.com', 
      phone: '+1111111111',
      address: 'Calle Cliente 111'
    },
    { 
      name: 'Pedro Martínez', 
      email: 'pedro@email.com', 
      phone: '+2222222222',
      address: 'Av. Comprador 222'
    },
    { 
      name: 'Laura Sánchez', 
      email: 'laura@email.com', 
      phone: '+3333333333',
      address: 'Plaza Consumidor 333'
    }
  ]

  await insertData('customers', customersData, 'Insertando clientes')

  // 6. Verificar datos insertados
  console.log('\n📊 VERIFICANDO DATOS INSERTADOS')
  console.log('-' .repeat(40))
  
  const tables = ['roles', 'permissions', 'categories', 'suppliers', 'customers']
  
  for (const table of tables) {
    try {
      const { data, error } = await supabase
        .from(table)
        .select('*', { count: 'exact' })
        .limit(1)

      if (error) {
        console.log(`❌ Error verificando ${table}: ${error.message}`)
      } else {
        console.log(`✅ Tabla ${table}: ${data?.length || 0} registros encontrados`)
      }
    } catch (err: any) {
      console.log(`❌ Error verificando ${table}: ${err.message}`)
    }
  }

  // Resumen final
  console.log('\n📋 RESUMEN FINAL')
  console.log('=' .repeat(60))
  console.log(`✅ Operaciones exitosas: ${successCount}`)
  console.log(`❌ Errores: ${errorCount}`)
  console.log(`📊 Tasa de éxito: ${successCount > 0 ? Math.round((successCount / (successCount + errorCount)) * 100) : 0}%`)

  if (errors.length > 0) {
    console.log('\n❌ ERRORES DETALLADOS:')
    errors.forEach((error, index) => {
      console.log(`${index + 1}. ${error}`)
    })
  }

  if (errorCount === 0) {
    console.log('\n🎉 ¡DATOS INSERTADOS EXITOSAMENTE!')
    return { success: true, successCount, errorCount }
  } else {
    console.log('\n⚠️ Inserción completada con errores')
    return { success: false, successCount, errorCount, errors }
  }
}

// Ejecutar el script
insertDataNatively()
  .then((result) => {
    if (result.success) {
      console.log('✅ Script completado exitosamente')
      process.exit(0)
    } else {
      console.log('⚠️ Script completado con errores')
      process.exit(1)
    }
  })
  .catch((error) => {
    console.error('❌ Error crítico:', error)
    process.exit(1)
  })