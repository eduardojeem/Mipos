...
-- Fecha: 2024-12-17 (Versión corregida)
...
-- 7. Crear función para auditoría automática
...
UPDATE roles SET priority = 50 WHERE name ILIKE '%employee%' AND (priority IS NULL OR priority = 50);
UPDATE roles SET priority = 25 WHERE name ILIKE '%cashier%' AND (priority IS NULL OR priority = 50);
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'ERROR: Columna parent_role_id no se creó';
    ELSE
        RAISE NOTICE 'OK: Columna parent_role_id creada';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'ERROR: Tabla role_audit_log no se creó';
    ELSE
        RAISE NOTICE 'OK: Tabla role_audit_log creada';
    END IF;
    
    -- Verificar que las funciones se crearon
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.routines 
        WHERE routine_name = 'get_role_statistics'
    ) THEN
        RAISE NOTICE 'ERROR: Función get_role_statistics no se creó';
    ELSE
        RAISE NOTICE 'OK: Función get_role_statistics creada';
    END IF;
END $$;
